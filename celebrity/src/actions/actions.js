...
export const createCeleb = celeb => dispatch => {
    console.log(celeb)
    dispatch({type: CREATE_CELEB_START})
    axiosWithAuth().post('/celeb', celeb)
    .then(res => {
        console.log(res)
        dispatch({type: CREATE_CELEB_SUCCESS, payload: {id: res.data.id, celebname: res.data.celebname, image_url: res.data.image_url, factoid: res.data.factoid, birthyear: res.data.birthyear, alive: res.data.alive}})
    })
}

export const editCeleb = celeb => dispatch => {
    dispatch({type: EDIT_CELEB_START})
    axiosWithAuth().put('/celeb', celeb)
    .then(res => {
        console.log(res)
        dispatch({type: EDIT_CELEB_SUCCESS, payload: {id: res.data.id, celebname: res.data.celebname, image_url: res.data.image_url, factoid: res.data.factoid, birthyear: res.data.birthyear, alive: res.data.alive}})
    })
}